...
export default function TomlToJsonPage() {
  ...
  const sampleToml = `# Sample TOML configuration
...
role = "editor"`;
  ...
  return (
    <ToolPage icon={ArrowRight} title="TOML → JSON Converter" description="Convert TOML configuration files to JSON format." pageTitle="TOML → JSON Converter — Free, Offline | Anatini.dev" metaDescription={getToolMetaDescription("toml-to-json")} seoContent={getToolSeo("toml-to-json")}>
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border border-border bg-muted/30 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setInput(sampleToml)}>Load Sample</Button>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">TOML Input</label>
        ...
        <CrossToolLinks format="toml" excludeRoute="/toml-to-json" />
      </div>
    </ToolPage>
  );
}
